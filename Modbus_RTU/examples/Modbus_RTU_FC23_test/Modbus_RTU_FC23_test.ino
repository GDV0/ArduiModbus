...
/*
  Modbus_RTU library
  Example of Mobus RTU Function code 23: Read/Write Registers
...
void setup()
{
  ...
  // Modbus test type 
  Serial.println("");
  Serial.println("   Test Function code 23: Read/Write Registers");
  Serial.println("   -------------------------------------------");
}
...
{
  Serial.println("");
  Serial.println("  --> Write 10 registers from address 1");
  Serial.println("  --> Read 10 registers from address 1");
  ...
  myData.data[5] =0x6666;
  myData.data[6] =0x7777;
  myData.data[7] =0x8888;
  myData.data[8] =0x9999;
  myData.data[9] =0x1010;
  myData.length =10;
	
  // Build Client request
  Serial.println("");
  Serial.println("  Request sent by the Client");
  myClient.Client_ReadWriteMultipleRegisters(5, 1, 10, 1, &myData, &myFrame);
...
t_status Modbus_CB_SetRegister(unsigned short Addr, int* Value)
{
  ...
  // *Addr = Value To be defined by application
  switch(Addr)
  {
    case 1:
    case 2:
